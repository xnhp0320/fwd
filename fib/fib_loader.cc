...
absl::Status LoadFibFileToTbm(const std::string& file_path, FibTbm* tbm,
                               uint32_t* rules_loaded) {
  if (tbm == nullptr) {
    return absl::InvalidArgumentError("tbm pointer is null");
  }
  ...
  while (std::getline(file, ip_line)) {
    ...
    // tbm_insert expects the IP in host byte order.
    uint32_t ip_host = ntohl(addr.s_addr);

    FibCidr cidr_val{.ip = ip_host, .cidr = static_cast<uint32_t>(cidr)};
    c3fault_t fault = tbm_insert(tbm, cidr_val, 0);
    if (fault != nullptr) {
      return absl::InternalError(absl::StrCat(
          "tbm_insert failed for ", ip_line, "/", cidr));
    }
    ++count;
  }

  std::cout << "TBM FIB loaded: " << count << " prefixes from " << file_path
            << "\n";
  if (rules_loaded != nullptr) {
    *rules_loaded = count;
  }
