...
absl::Status TbmForwardingProcessor::check_impl(
    const std::vector<dpdk_config::QueueAssignment>& /*rx_queues*/,
    const std::vector<dpdk_config::QueueAssignment>& tx_queues) {
  if (tx_queues.empty()) {
    return absl::InvalidArgumentError(
        "tbm_forwarding requires at least one TX queue");
  }
  return absl::OkStatus();
}

void TbmForwardingProcessor::ExportProcessorData(ProcessorContext& ctx) {
  if (ctx.tbm_table) {
    tbm_table_ = static_cast<FibTbm*>(ctx.tbm_table);
  }
}

absl::Status TbmForwardingProcessor::CheckParams(
...
void TbmForwardingProcessor::process_impl() {
  ...
  for (const auto& rx : config().rx_queues) {
    ...
    // Parse each packet and perform TBM lookup for IPv4 packets.
    batch.PrefetchForEach<3>([&](rxtx::Packet& pkt) {
      ...
      if (tbm_table_ != nullptr && !meta.IsIpv6()) {
        uint32_t next_hop = 0;
        tbm_lookup(&next_hop, tbm_table_, ntohl(meta.dst_ip.v4));
